...
class ProjectPythonAnalyzer:
    ...
    def _eval(
        ...
        if isinstance(expression, ast.NamedExpr):
            value = self._eval(expression.value, environment, function, callstack)
            self._assign_target(expression.target, value, environment, function, expression, expression.value, callstack)
            return value
        if isinstance(expression, ast.Attribute):
            ...
            return precise if location and environment.contains(location) else precise.merge(self._eval(expression.value, environment, function, callstack))
        if isinstance(expression, ast.Subscript):
            ...
            if location and environment.contains(location):
                return precise
            base = self._eval(expression.value, environment, function, callstack)
            key = self._eval(expression.slice, environment, function, callstack)
        ...
        if isinstance(expression, (ast.JoinedStr, ast.BinOp, ast.BoolOp, ast.List, ast.Tuple, ast.Set)):
            ...
            for child in children:
                value = value.merge(self._eval(child, environment, function, callstack))
        ...
        if isinstance(expression, (ast.ListComp, ast.SetComp, ast.GeneratorExp)):
            value = self._eval(expression.elt, environment, function, callstack)
            for generator in expression.generators:
                value = value.merge(self._eval(generator.iter, environment, function, callstack))
        ...
        if isinstance(expression, ast.Call):
            ...
            if (
                ...
                child_environment = self._call_environment(expression, environment, function, target, callstack)
                returned, _ = self._run_block(
                    target.body,
                    child_environment,
                    target,
                ...
                if sanitizer.state is not SanitizerState.ABSENT:
                    returned = returned.sanitized(
                        sanitizer,
                        self._node(function, "sanitizer", f"{sanitizer.state.value}: {sanitizer.reason}", expression, name),
                    )
                return returned
            value = TaintValue()
            for argument in expression.args:
                value = value.merge(self._eval(argument, environment, function, callstack))
            for keyword in expression.keywords:
                value = value.merge(self._eval(keyword.value, environment, function, callstack))
            value = value.through(self._node(function, "propagator", f"call through {name}", expression, name))
            if sanitizer.state is not SanitizerState.ABSENT:
        ...
        if isinstance(expression, ast.IfExp):
            return self._eval(expression.body, environment, function, callstack).merge(
                self._eval(expression.orelse, environment, function, callstack)
            )
    ...
    def _call_environment(
        ...
        environment = caller_environment.fork()
        ...
        for parameter, expression in zip(positional_parameters, call.args, strict=False):
            value = self._eval(expression, caller_environment, caller, callstack)
            if value.labels:
                value = value.through(
                    self._node(
                        target,
        ...
        for keyword in call.keywords:
            if keyword.arg and keyword.arg in by_name:
                value = self._eval(keyword.value, caller_environment, caller, callstack)
                if value.labels:
                    value = value.through(
                        self._node(
                            target,
    ...
    @staticmethod
    def _target_applicable(rule: Rule, function: ProjectFunction) -> bool:
        if not rule.targets:
            return True
        ...
        if targets.issubset({"skill", "plugin", "skill_manifest"}):
            return is_skill
        if targets.issubset({"mcp_client"}):
            return any(token in low_source for token in ("stdio_client", "streamable_http_client", "clientsession", "mcp.client"))
        if targets.issubset({"mcp_server"}):
            return is_mcp
        if targets.issubset({"llm_client"}):
            return any(token in low_source for token in ("openai", "anthropic", "generativemodel", "chatgoogle", "azureopenai"))
        if targets and all(target.startswith("mcp") or target == "mcp" for target in targets):
            return is_mcp
        return True
    ...
    def _rules_for(
        ...
        for rule in self.rules:
            ...
            if not self._target_applicable(rule, function):
                continue
            if not rule.sources or not rule.sinks:
                continue
    ...
    def _find_sinks_in_expression(
        ...
        findings: list[Finding] = []
        for call in (node for node in ast.walk(expression) if isinstance(node, ast.Call)):
            call_name = self.index.canonical(call.func, function)
            sinks = self._sink_kinds(call_name)
            argument_taint = TaintValue()
            for argument in call.args:
                argument_taint = argument_taint.merge(self._eval(argument, environment, function, callstack))
            for keyword in call.keywords:
                argument_taint = argument_taint.merge(self._eval(keyword.value, environment, function, callstack))
            ...
            target = self.index.resolve_function(call, function)
            if target and target.qualname not in callstack and self.session.allow_context(target.qualname, (*callstack, target.qualname)):
                child_environment = self._call_environment(call, environment, function, target, callstack)
                _, nested = self._run_block(
                    target.body,
                    child_environment,
                    target,
                    (*callstack, target.qualname),
    ...
    def _assign_target(
        ...
        if isinstance(target, (ast.Tuple, ast.List)):
            ...
            for index, element in enumerate(target.elts):
                ...
                child_value = self._eval(child_expression, environment, function, callstack) if child_expression else value
                self._assign_target(element, child_value, environment, function, statement, child_expression, callstack)
            return
        ...
        if isinstance(source_expression, ast.Dict):
            for key, child_expression in zip(source_expression.keys, source_expression.values, strict=False):
                ...
                child_value = self._eval(child_expression, environment, function, callstack)
    ...
    def _run_block(
        ...
        for statement in body:
            if isinstance(statement, (ast.Assign, ast.AnnAssign)):
                ...
                if collect_findings:
                    findings.extend(self._find_sinks_in_expression(value_node, environment, function, callstack))
                value = self._eval(value_node, environment, function, callstack)
                targets = statement.targets if isinstance(statement, ast.Assign) else [statement.target]
                for target in targets:
                    self._assign_target(target, value, environment, function, statement, value_node, callstack)
                ...
                if collect_findings:
                    findings.extend(self._find_sinks_in_expression(statement.value, environment, function, callstack))
                self._eval(statement.value, environment, function, callstack)
            elif isinstance(statement, ast.Return):
                if collect_findings and statement.value is not None:
                    findings.extend(self._find_sinks_in_expression(statement.value, environment, function, callstack))
                return_value = self._eval(statement.value, environment, function, callstack)
                ...
                environment.edges.append(return_edge)
