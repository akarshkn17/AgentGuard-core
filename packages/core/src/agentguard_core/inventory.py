...
from .frameworks import (
    AGENT_CONSTRUCTORS, MCP_PROMPT_DECORATORS, MCP_RESOURCE_DECORATORS,
    MCP_SERVER_CONSTRUCTORS, MEMORY_CONSTRUCTORS, MODEL_CONSTRUCTORS,
    RETRIEVER_HINTS, TOOL_DECORATORS, VECTOR_CONSTRUCTORS,
)
...
CATEGORY_BY_TYPE={
    ...
    "memory":"memory_state","checkpoint_store":"memory_state","prompt":"prompting","guardrail":"safety","observability":"operations","deployment":"operations","identity":"operations",
...
FRAMEWORK_PACKAGE_KEYS={"langchain":("langchain",),"langgraph":("langgraph",),"openai-agents":("openai-agents","openai_agents"),"mcp":("mcp",),"google-adk":("google-adk","google_adk"),"crewai":("crewai",),"autogen":("autogen","pyautogen"),"llamaindex":("llama-index","llama_index"),"semantic-kernel":("semantic-kernel",)}
...
class InventoryDiscoverer:
    ...
    def _version(self,etype:str,content_hash:str,explicit:str="",model_id:str="")->tuple[str,str,str,dict[str,Any]]:
        if model_id:return model_id,"model_identifier","model-id",{"model_id":model_id}
    ...
    def _relationship(self,s:InventoryEntity,relation:str,t:InventoryEntity,path:Path,line:int,attrs:dict[str,Any]|None=None)->None:
        self.relationships.append(Relationship(s.entity_id,relation,t.entity_id,str(path),line,{"resolution":"exact",**(attrs or {})}))
    ...
    def _discover_assignments(self,index:ProjectIndex,module:ProjectModule)->None:
        ...
        for node in module.tree.body:
            ...
            elif tail in {x.split('.')[-1] for x in MODEL_CONSTRUCTORS}:et="model"
            ...
            for k in ("model","model_name","model_id","modelId","base_url","endpoint","azure_endpoint","url","version","agent_version"):
                v=_string(kw.get(k));
                if v:attrs[k]=v
            explicit=attrs.get("version") or attrs.get("agent_version") or ""; model_id=(attrs.get("model") or attrs.get("model_name") or attrs.get("model_id") or attrs.get("modelId") or "") if et=="model" else ""
    ...
    def _resolve(self,module:ProjectModule,node:ast.AST,etype:str="")->InventoryEntity|None:
        if isinstance(node,ast.Name):return self.variables.get(module.module,{}).get(node.id) or (self._infer_function_entity(module,node,etype) if et else None)
        return None
    ...
    def _discover_js_ts(self,paths:list[Path])->None:
        for path in paths:
            ...
            text=path.read_text(encoding="utf-8",errors="ignore")
            for m in re.finditer(r"(?m)\b(?:const|let|var)\s+(\w+)\s*=\s*new\s+(Agent|AssistantAgent|MCPServer|FastMCP|OpenAI|Anthropic)\s*\((.*?)\)",text,re.S):
    ...
    def discover(self)->tuple[list[InventoryEntity],list[Relationship]]:
        ...
        for m in index.modules.values():self._discover_functions(index,m);self._discover_assignments(index,m)
        for m in index.modules.values():self._discover_relationships(index,m)
        self._infer_capabilities(index);self._discover_skill_manifests(paths);self._discover_agent_manifests(paths);self._discover_js_ts(paths);self._add_dependencies()
