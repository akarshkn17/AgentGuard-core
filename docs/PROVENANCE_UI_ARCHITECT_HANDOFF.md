...
The graph exporter transforms the same result into `agentguard-provenance-graph/1.0`. The UI should consume these contracts; it should not parse source code itself.
...
version         = human-readable declared/project/model version
version_source  = agent_manifest | project_manifest | constructor | model_id | git_tag | git_commit | content_hash
version_scheme  = declared | semver | model-id | git-tag | git-commit | content-hash
...
**Do not use `framework_version` as the agent version.**
...
UI should use `kind` for the broad renderer and `subtype` for icons/filters.
...
The graph output gives one path object per finding today:
...
The UI can highlight exactly `node_ids` and `edge_ids` without recomputing a graph traversal.
